...
- **`--dry-run`/`DRY_RUN`은 '과금 0'이 계약이다.** 인스턴스·엔드포인트만 막고 끝내지 말 것 —
  **토큰당 과금 API**(Bedrock Converse, 임베딩, LLM-as-judge)도 함께 막는다. **건수를 줄이는 것은
  가드가 아니다**: 100건→10건은 호출이 10회 일어나고 그만큼 청구된다. 구현 후 반드시 **호출 경로를
  코드로 확인**하고(`grep`으로 client 호출이 가드 안쪽인지), dry-run 로그에 진행 바나 지연이 보이면
  실제로 부르고 있다는 신호다. 대안: 시드/기존 산출물을 복제해 **형식만** 검증하거나 무료 경로로 대체.
