...
AWS ML 실습 코드와 노트북을 만들 때 **환경에 맞는 테스트 사다리**를 함께 제공한다. 항상 적용.
(코드 스타일은 [[code-style]], 작성 규칙과 cleanup은 [[aws-authoring]], SDK와 API 사실은 [[fact-integrity]]로 검증.)

## 원칙
- **테스트는 환경 능력에 맞춘다.** 코드에 환경 감지(가용 디바이스, 자격증명, 리전)를 넣고, 능력별로 다른 경로를 태운다.
- **비용과 과금을 항상 가드.** GPU 인스턴스와 엔드포인트와 클러스터는 안 지우면 계속 과금되므로 cleanup 필수, 무거운 셀엔 경고.
- **작게 시작.** 전체 학습 전에 1-step/1-batch로 파이프라인이 도는지부터.
- **`--dry-run`과 `DRY_RUN`은 '과금 0'이 계약이다.** 인스턴스와 엔드포인트만 막고 끝내지 말고,
  **토큰당 과금 API**(Bedrock Converse, 임베딩, LLM-as-judge)도 함께 막는다. **건수를 줄이는 것은
  가드가 아니다**: 100건에서 10건으로 줄여도 호출이 10회 일어나고 그만큼 청구된다. 구현 후 반드시
  **호출 경로를 코드로 확인**하고(`grep`으로 client 호출이 가드 안쪽인지), dry-run 로그에 진행 바나
  지연이 보이면 실제로 부르고 있다는 신호다. 대안은 시드나 기존 산출물을 복제해 **형식만** 검증하거나
  무료 경로로 대체하는 것.

## 테스트 사다리 (환경별)
1. **GPU 있는 개발환경** → **샘플 데이터로 실제 실행**. 소형 모델과 부분 데이터로 학습과 추론이 end-to-end로 도는지. 예: `torch.cuda.is_available()` 확인 → 작은 batch로 몇 step.
...
3. **SageMaker AI 학습/추론** → **직접 인스턴스 띄워 테스트**. `local mode`(로컬 컨테이너)로 먼저 검증 → 그다음 작은 `instance_type`으로 실제 Training Job이나 Endpoint. `estimator.fit()`과 `deploy()` 후 반드시 teardown(`predictor.delete_endpoint()`).
4. **Docker 이미지** → **로드해서 테스트**. DLC 또는 커스텀 이미지를 `docker pull`/`docker run`으로 로컬 로드 → 컨테이너 안에서 스크립트 스모크 실행 → (필요시) ECR push 전 검증.
5. **엔드포인트 호출** → 배포 후 `invoke_endpoint`(boto3) 또는 `Predictor.predict()`로 실제 응답과 지연과 payload 한계 확인. (Serverless엔 GPU가 없으므로 LLM은 Real-time GPU로.)
6. **Agentic AI 연동** → 엔드포인트나 모델을 **Strands SDK, LangGraph, Bedrock AgentCore**와 연동해 에이전트 루프로 호출 테스트. 툴 호출과 멀티스텝과 상태를 검증.
...
- **Strands SDK, LangGraph, Bedrock AgentCore**의 정확한 API와 클래스와 버전은 빠르게 바뀌므로, 코드화 전 **공식 문서와 repo로 검증**([[fact-integrity]], `aws-fact-checker`). 미검증 시그니처는 `# TODO verify` 주석.
- 연동 테스트도 과금이 발생하므로(모델 호출, 엔드포인트) 최소 호출과 cleanup.
- 시크릿(API key, role)은 env나 플레이스홀더로, 코드와 노트북 출력에 노출 금지.

## CloudWatch 관측 다이렉트 링크 (특히 노트북)
SageMaker Training Job과 Endpoint를 호출하거나 배포하는 코드는 **CloudWatch로 상황(로그, 메트릭)을 바로 볼 수 있는 다이렉트 링크를 자동으로 출력**한다. 특히 **Jupyter 노트북에서는 클릭 가능한 링크**(`IPython.display.HTML`)로.

- **언제**: `estimator.fit()` 직후(학습 로그), `deploy()`/엔드포인트 생성 직후(추론 로그+메트릭), `invoke_endpoint` 테스트 시.
- **무엇을 링크**: ① SageMaker 콘솔의 해당 Job이나 Endpoint 페이지 ② CloudWatch Logs 로그그룹 ③ (엔드포인트) CloudWatch Metrics(Invocations, ModelLatency, OverheadLatency, 4xx/5xx).
- **로그그룹 규약**: 학습 = `/aws/sagemaker/TrainingJobs`(스트림 `<job-name>/…`), 엔드포인트 = `/aws/sagemaker/Endpoints/<endpoint-name>`.
- ⚠️ **콘솔 URL 형식은 현재 기준**이며 바뀔 수 있다(특히 Logs 경로는 `/` → `$252F` 이중 인코딩). 배포 전 실제 링크 클릭 확인. 리전과 계정은 세션에서 동적으로(하드코딩 금지).
...
## 산출물 형태
- 각 실습 코드에 **"테스트 방법" 섹션**(위 사다리 중 해당 환경 경로), 예상 소요와 과금 경고, cleanup.
- SageMaker 학습과 추론 코드에는 **CloudWatch 다이렉트 링크 출력**(노트북은 클릭 링크, 스크립트는 print).
