...
- **1차 소스 우선.** AWS는 공식 문서(docs.aws.amazon.com)와 공식 GitHub raw. 논문은 arXiv, ACL Anthology, venue proceedings 원문. 마케팅 블로그와 2차 요약은 보조.
...
- **위임**: AWS 사실은 `aws-fact-verify`(스킬)나 `aws-fact-checker`(서브에이전트)로. 논문 인용은 `arxiv-verify`나 `citation-workflow`로. 확인 불가한 인용은 PLACEHOLDER와 TODO로 표시하고 절대 지어내지 않는다.
