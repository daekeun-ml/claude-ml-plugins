...
- 🔴 **IN_PROCESS 함정(실측 refute)**: `InProcessServer`는 `model=<HF id 문자열>`을 `transformers.pipeline` 또는 `SentenceTransformer`(임베딩)로만 로드 시도(sagemaker/serve/model_server/in_process_model_server/app.py). 즉 **분류·임베딩 등 경량 모델 전용**. 생성형 멀티모달 LLM(gemma-4)은 pipeline이 `AnyToAnyPipeline`으로 잡혀 `librosa` 요구→실패, SentenceTransformer 폴백도 실패(`UnboundLocalError`). LLM을 IN_PROCESS로 띄우려면 `InferenceSpec`(load/invoke) 직접 구현 필요(사실상 엔진 재구현).
- 🔴 **LOCAL_CONTAINER 함정(실측, gemma-4 E4B)**: (a) vLLM DLC를 `image_uri`만 주면 passthrough라 `model_server=None` → `local_container_mode.create_server`엔 **VLLM 분기가 없어**(TRITON/DJL_SERVING/TORCHSERVE/TGI/MMS/TF/TEI만) `None.logs()` 크래시. (b) `model_server=DJL_SERVING` + DJL LMI 이미지로 우회 시: 모델은 `model_path/code/`에 **실파일**로 둬야 마운트됨(심링크는 컨테이너 안에서 깨짐), docker-py 기본 타임아웃 60s는 38GB 이미지에 부족(`container_timeout_in_seconds` 상향). 그래도 **LMI 0.36.0 번들 vLLM은 E4B의 이종 레이어(레이어 0~23만 `k_norm` 보유; 원본 모델 구조가 그러함)를 미완전 지원**해 `weights not initialized: layers.24~41.self_attn.k_norm` ValueError로 엔진 초기화 실패. arch 인식(Gemma4ForCausalLM)은 됨 → 순수 미지원이 아니라 **번들 vLLM 버전 미성숙**. → gemma-4는 vLLM≥0.19(권장 0.25.x) 필요.
- **결론**: SLM/LLM(특히 gemma-4) 로컬 검증은 SDK 로컬 모드가 아니라 **`vllm serve` 직접**(최신 vLLM)으로. 클라우드는 vLLM DLC 0.25.1 endpoint.
